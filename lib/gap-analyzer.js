...
async function runPerDomainReassembly({
  ...
  // p4r-7 Step 0 (option a — reassembly mints): backstop gap_id + analyzed_at
  // for any per-domain analyzer that left them unset. Per-domain analyzers
  // that mint their own are respected (only fill if absent). Format mirrors
  // single-pass (line 199) for traceability parity:
  // `urn:llm-ops:cortex-gap:<epoch_ms>-<idx>-<6-char-rand>`. analyzed_at uses
  // a single ISO-stamp per reassembly call (matches single-pass nowIso pattern).
  const reassemblyAnalyzedAt = new Date().toISOString();
  const reassemblyMintEpoch = Date.now();
  allGaps.forEach((gap, idx) => {
    if (!gap.gap_id) {
      gap.gap_id = `urn:llm-ops:cortex-gap:${reassemblyMintEpoch}-${idx}-${Math.random().toString(36).slice(2, 8)}`;
    }
    if (!gap.analyzed_at) {
      gap.analyzed_at = reassemblyAnalyzedAt;
    }
  });
