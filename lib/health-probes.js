 ...
 * @param {{ snapshot: (ms: number) => object, size: () => number }} [deps.assessmentRing]
 *   Optional ring buffer (C2A-04). When present, /introspect includes
 *   `degraded_ratio` with 1h and 24h windowed snapshots.
 * @returns {() => Promise<object>} flat introspectCheck function
 */
export function buildIntrospectCheck({ cadence, assessmentLoop, goalHistory, missionLoader, assessmentRing, llmLoader }) {
  return async function introspectCheck() {
    ...
    if (assessmentRing) {
      base.degraded_ratio = {
        '1h': assessmentRing.snapshot(3600000),
        '24h': assessmentRing.snapshot(86400000),
        ring_capacity: 1440,
        ring_size: assessmentRing.size(),
      };
    }

    // MP-CONFIG-1 R7 — flat per bug #9; consumed by Axon aggregator R8.
    if (llmLoader) {
      base.llm = llmLoader.introspect();
    }
