...
const sampleMspRow = {
  ...
  data: {
    ...
    status: 'active',
    hash: 'abcdef123',
    raw_text: '# Mission Statement Protocol — Coretex Agentic DIO\n\n## Purpose\n\nThe MSP is the operational constitution.',
...
const sampleBor = {
  version: '1.0.0',
  hash: '987fedcba',
  raw_text: '# Bill of Rights — Coretex Agentic DIO\n\n## Article 1: Agency',
...
test('loadMission composes both MSP and BoR on happy path', async () => {
  ...
  assert.equal(frame.msp.raw_text.startsWith('# Mission'), true);
  assert.equal(frame.bor.version, '1.0.0');
  assert.equal(frame.bor.raw_text.startsWith('# Bill of Rights'), true);
...
test('loadMission legacy-compat: msp_version concept without raw_text', async () => {
  const legacy = {
    ...sampleMspRow,
    data: { ...sampleMspRow.data, raw_text: undefined },
  };
  const loader = createMissionLoader({
...
test('loadMission serves from cache within TTL', async () => {
  ...
  const graph = {
    queryConcepts: async () => {
      callCount += 1;
      return { rows: [sampleMspRow], count: 1 };
    },
  };
  const loader = createMissionLoader({
    graphAdapter: graph,
...
test('concurrent MSP and BoR loads (Promise.all parallelism)', async () => {
  ...
  const arbiter = {
    getBoRRaw: async () => {
      borStartedAt = Date.now();
      await new Promise(r => setTimeout(r, 20));
      return sampleBor;
    },
  };
  const loader = createMissionLoader({ graphAdapter: graph, arbiterClient: arbiter });
  await loader.loadMission();
  // Both should start within a few ms of each other (parallel), not sequentially
  assert.ok(Math.abs(mspStartedAt - borStartedAt) < 10, 'MSP and BoR queries should run concurrently');
});
